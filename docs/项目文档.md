...
| 5 | **人脸检测**（可插拔适配器，未启用则跳过）生成候选模糊区域 | 检测异常不影响上传，转人工 |
| 6 | **车牌检测**（OCR 方案，可插拔适配器）生成候选模糊区域 | 同上 |
| 6.5 | **置信度分级**：≥ 自动阈值直接采纳；落在复核区间标记为疑难区域；低于复核阈值按误报丢弃 | — |
| 7 | 写入 `blur_regions`，来源标记 `auto`，疑难区域标记 `needs_review` | — |
...
```
processing ──► auto_clean        （检测器可用，未检测到敏感区域）
          ├─► auto_confirmed     （检测框全部高置信度，自动打码并直接放行）
          ├─► needs_manual       （存在疑难区域（中置信度），或检测器不可用）
          └─► failed             （处理失败，可重试）

needs_manual ──(审核员复核疑难区域/框选并应用模糊)──► auto_blurred / manual_blurred
...
置信度分级由两个阈值控制（`DETECTION_AUTO_THRESHOLD` / `DETECTION_REVIEW_THRESHOLD`）：
高置信度区域自动打码后系统直接放行，不占用审核人力；中置信度的疑难区域仍然打码（宁滥勿缺），
但图片进入 `needs_manual`，审核员只需复核这些疑难区域；低于复核阈值的框按误报丢弃。
打码策略是"宁滥勿缺"，误检的代价是图片局部被糊（体验问题），不会放大隐私风险。

检测器或阈值升级后，可通过 `POST /admin/media/rerun-detection` 批量重跑存量图片：
任务逐张入队由 worker 异步执行，默认只重跑机器处理过的状态（不动人工确认成果），
原图已清理的图片自动跳过。
...
- 自动检测出的区域以虚线高亮显示，疑难区域（中置信度）以红色虚线重点标出，审核员可逐条采纳或忽略；
