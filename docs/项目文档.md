...
| 5 | **调用已启用的检测适配器**（人脸 / 车牌 / 外部 HTTP 服务 / 插件），在清洗后的原图上跑检测 | 单个适配器异常不影响其他适配器，全部不可用则转人工 |
| 6 | 多适配器结果 NMS 去重，按置信度自动分级（见 5.4.2） | — |
| 7 | 高/中置信度框写入 `blur_regions`（`review_status` = trusted / pending），低置信度候选只入检测元数据 | — |
| 8 | 计算 `privacy_status` | 见 5.4.2 |
| 9 | 按最终区域从原图一次性渲染公开变体，原图写入私有存储 | — |

> 步骤 3 是**强制且不可跳过**的：即使用户上传的原图带 GPS，系统也绝不会把 GPS 透传给任何人。这是保护贡献者的第一道防线。

##### 可插拔检测适配器

检测器不再写死在流水线里，而是统一实现 `DetectionAdapter` 接口（`backend/src/services/detection/`）：

- **内置**：`face`（face-api）、`plate`（tesseract OCR），由 `DETECTION_ADAPTERS` 或旧开关 `ENABLE_FACE_DETECTION` / `ENABLE_PLATE_DETECTION` 启用；
- **外部服务**：`http` 适配器把图片 POST 到 `DETECTOR_HTTP_URL`，对端返回归一化框（支持像素坐标自动换算与严格校验）；
- **第三方插件**：`DETECTION_PLUGINS` 指向 npm 包或本地路径，模块默认导出 `(register) => register(adapter)` 即可挂载，无需改动主代码。

每个框记录 `detector`（产出适配器），每次检测把各适配器的可用性、命中数、失败原因与分级阈值快照写入 `detection_meta`，结果可追溯、可复现解释。

#### 5.4.2 置信度分级与隐私状态机

每个检测框按置信度（阈值 `DETECTION_AUTO_CONFIRM_THRESHOLD` / `DETECTION_REVIEW_THRESHOLD`，默认 0.85 / 0.55）分三级：

| 分级 | 区间 | 处理 |
|---|---|---|
| `trusted`（高置信度） | ≥ 0.85 | 自动模糊，**直接放行，不占人工**；`review_status=trusted` |
| `ambiguous`（疑难） | 0.55–0.85 | 默认先模糊（fail-safe），进入人工复核队列；`review_status=pending` |
| `candidate`（候选） | < 0.55 | 不打码，仅记录到检测元数据，要求人工肉眼抽查 |

```
processing ──► auto_clean        （适配器可用，零命中）
          ├─► auto_confirmed     （命中全部为高置信度，自动模糊并放行）
          ├─► auto_blurred       （有中置信度框或低置信度候选，待人工复核疑难）
          ├─► needs_manual       （没有任何适配器可用，整图人工兜底）
          └─► failed             （处理失败，可重试）

auto_blurred ──(疑难框被逐条裁定 accepted/dismissed 且无遗留候选)──► confirmed
needs_manual ──(审核员框选并应用模糊)──► manual_blurred ──(确认)──► confirmed
auto_*/manual_blurred ──(审核员整图确认)──────────────────────► confirmed
```

**只有 `auto_clean`、`auto_confirmed` 与 `confirmed` 状态的图片才允许随条目一同发布。**
中置信度框默认就模糊上了，所以 `auto_blurred` 不可发布是"等待放行确认"，而不是"裸露等待"。

##### 批量重跑存量图片

安装 / 升级适配器后可对历史图片补检，人工已裁定（accepted/dismissed）的框与手动框受保护，机器只重新评估尚未裁定的位置；重检导致的状态收紧自动生效（已放行图也可能被降回复核态）：

- 管理接口：`POST /api/v1/moderation/privacy/redetect`（admin，支持按状态/UUID/时间筛选与限量）；
- 运维脚本：`npm run redetect -- --statuses needs_manual,auto_blurred --limit 500`（`--inline` 绕过队列）；
- 复核队列：`GET /api/v1/moderation/privacy/review-queue`，默认只返回需要人工的图片。
